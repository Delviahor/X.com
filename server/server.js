...
app.get('/historial', (req, res) => {
    ...
    // Consulta para obtener el historial de transferencias usando el nombre de usuario
    const getHistorialQuery = `
        SELECT t.fecha, t.remitente_id, t.destinatario_id, t.monto
        FROM transferencias t
        WHERE t.remitente_id = ? OR t.destinatario_id = ?
        ORDER BY t.fecha DESC
    `;

    // Ejecutar la consulta con el nombre de usuario en lugar de IDs
    db.all(getHistorialQuery, [username, username], (err, rows) => {
        if (err) {
            console.error(err.message);
